  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Animated.View entering={FadeInUp} style={styles.content}>
        ...
        <Modal visible={showParticipantModal} transparent={true} animationType="none">
          <KeyboardAvoidingView 
          ...
          >
            <Animated.View entering={FadeInUp} style={styles.modalContent}>
              ...
              <Text style={styles.modalTitle}>{selectedBooking ? t('advancedBooking.editPlayersTitle') : t('advancedBooking.addPlayersTitle')}</Text>
              <View style={styles.bookingInfoContainer}>
                <View style={styles.bookingInfoItem}>
                  <Icon name="sports-tennis" size={16} color={COURT_BLUE} />
                  <Text style={styles.bookingInfoText}>
                    {courts[selectedCourtIndex]?.name}
                  </Text>
                </View>
                <View style={styles.bookingInfoItem}>
                  <Icon name="schedule" size={16} color={COURT_BLUE} />
                  <Text style={styles.bookingInfoText}>
                    {selectedStart ? formatToCEST(selectedStart) : ''} - {selectedStart ? formatToCEST(addMinutes(selectedStart, duration)) : ''}
                  </Text>
                </View>
              </View>
                ...
                {selectedParticipants.map(id => {
                  ...
                  return (
                    <Animated.View entering={FadeIn} key={id} style={styles.chip}>
                      <Text style={styles.chipText}>{member?.firstName} {member?.lastName}</Text>
                      <TouchableOpacity onPress={() => removeParticipant(id)} accessibilityRole="button">
              ...
              <FlatList
                ...
                renderItem={({ item }) => {
                  if (item.id === Number(user?.id)) return null;
                  ...
                  return (
                    <Animated.View entering={FadeInLeft}>
                      <TouchableOpacity 
                        ...
                        style={[styles.participantItem, item.id && selectedParticipants.includes(item.id) ? styles.selectedParticipantItem : {}]}
                        accessibilityRole="button"
                        accessibilityState={{ disabled: isUnregistered }}
                      >
                        <View style={styles.participantInfo}>
                          <Text style={[styles.participantName, isUnregistered ? styles.disabledText : {}]}>
                            {item.id && item.firstName && item.lastName ? `${item.firstName} ${item.lastName}` : item.email} 
                            {isUnregistered ? t('advancedBooking.pendingMember') : ''}
                          </Text>
                          {item.id && !item.bookable && item.status === 'approved' && <Text style={styles.bookableBadge}>{t('advancedBooking.limitReached')}</Text>}
                        </View>
                        {item.id && selectedParticipants.includes(item.id) ? (
                          <Icon name="check-circle" size={20} color={ACE_GREEN} />
                        ) : null}
                      </TouchableOpacity>
                    </Animated.View>
                  );
              ...
              <View style={styles.modalBottomContainer}>
                <TouchableOpacity 
                  ...
                  style={[
                    ...
                    (!selectedStart || selectedParticipants.length === 0) && styles.disabledButton
                  ]}
                  disabled={!selectedStart || selectedParticipants.length === 0}
                >
                  <Text style={[styles.buttonText, { color: WHITE_LINES }]}>
                    {selectedBooking ? t('advancedBooking.updateNow') : t('advancedBooking.bookNow')}
                  </Text>
                </TouchableOpacity>
              </View>
            </Animated.View>
