...
GoogleSignin.configure({
  webClientId: '596452790791-r3tgi1q62apn42iukrit021hn2ldldq1.apps.googleusercontent.com', // Ersetze mit Web Client ID
  iosClientId: '596452790791-r3tgi1q62apn42iukrit021hn2ldldq1.apps.googleusercontent.com', // iOS Client ID
});
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    try {
      ...
      if (error.code === statusCodes.SIGN_IN_CANCELLED) {
        ...
        errorMessage = error.response
          ? `Google login failed: ${error.response.status} - ${error.response.data?.error || 'No error message'}`
  ...
  const registerWithGoogle = async () => {
    try {
      ...
      console.log('AuthContext: Google Sign-In successful for registration, ID token:', idToken);
      ...
      console.log('AuthContext: Google register response:', response.data);
      const { access_token, ...userData } = response.data;
      setIsLoading(true);
      setToken(access_token);
      await AsyncStorage.setItem('token', access_token);
      ...
      if (error.code === statusCodes.SIGN_IN_CANCELLED) {
        ...
        errorMessage = error.response
          ? `Google registration failed: ${error.response.status} - ${error.response.data?.error || 'No error message'}`
